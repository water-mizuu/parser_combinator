...
class ExpressionBuilder<T> {
  ...
  Parser<T> build() {
    ...
    Parser<T> built =
        expressionGroups.fold(minimum, (Parser<T> a, ExpressionGroup<T> group) => group.build(a)..pegMemoize = true);
...
class ExpressionGroup<T> {
  ...
  List<Parser<T>> _surround = <Parser<T>>[];
  ...
  ExpressionGroup<T> surround<L extends Object, R extends Object>(
    ...
    T Function(L left, T value, R right) handler,
      ...
      this
        .._surround
            .add((left & mirror & right).map((List<Object?> v) => handler(v[0].cast(), v[1].cast(), v[2].cast())));

  List<Parser<InfixWrapper<T, void>>> _left = <Parser<InfixWrapper<T, void>>>[];
  Parser<T> _buildLeft(Parser<T> previous) => _left.isEmpty
      ? previous
      : (previous & (_left.choice() & previous).star()).map((List<Object?> $) {
          T head = $[0].cast<T>();
          List<Object?> tail = $[1].cast<List<List<Object?>>>().expand((List<Object?> v) => v).toList();
  ...
  ExpressionGroup<T> left<O>(Parser<O> operator, MonoidInfixHandler<T, O> handler) =>
      this.._left.add(operator.map((O result) => InfixWrapper<T, O>(result, handler)));

  List<Parser<InfixWrapper<T, void>>> _right = <Parser<InfixWrapper<T, void>>>[];
  Parser<T> _buildRight(Parser<T> previous) => _right.isEmpty
      ? previous
      : (previous & (_right.choice() & previous).star()).map((List<Object?> $) {
          T head = $[0].cast<T>();
          List<Object?> tail = $[1].cast<List<List<Object?>>>().expand((List<Object?> v) => v).toList();
          List<Object?> flattened = <Object?>[head, ...tail];
  ...
  ExpressionGroup<T> right<O>(Parser<O> operator, MonoidInfixHandler<T, O> handler) =>
      this.._right.add(operator.map((O result) => InfixWrapper<T, O>(result, handler)));

  List<Parser<T>> _atomic = <Parser<T>>[];
  ...
  List<Parser<PrefixWrapper<T, void>>> _pre = <Parser<PrefixWrapper<T, void>>>[];
  ...
  final List<Parser<PostfixWrapper<T, void>>> _post = <Parser<PostfixWrapper<T, void>>>[];
